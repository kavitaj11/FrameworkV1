...
public class ShopByBrandsPageObjects extends MainController{
	...
	public ShopByBrandsPageObjects verifyTitleOfTheBrand(String nameOfTheBrand) throws Exception{
		Thread.sleep(2500);;
		Assert.assertTrue(driver.getTitle().trim().contains(nameOfTheBrand),"The title does not contain the brand that was clicked. The title is "+driver.getTitle().trim()+"."+"Asserting with data : "+nameOfTheBrand + " | "+setUp.getProductName().toUpperCase()+".");
